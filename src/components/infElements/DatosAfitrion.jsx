...
function DatosAnfitrion({ id, section }) {
  ...
  const [showModalCant, setShowModalCant] = useState(false);
  const [showModalCali, setShowModalCali] = useState(false);
  const [showModalSeg, setShowModalSeg] = useState(false);
  ...
  const [opinion, setOpinion] = useState({
    id_usuario: user.id,
    rol_usuario: user.rol,
    id_negocio: anfitrion.id,
    rol_receptor: anfitrion.rol,
    tipo: "",
    valuacion: ""
  });
  ...
  useEffect(() => {
    setUserOpinions(allOpinions.filter((opinion) => (opinion.id_usuario === user.id)))
  }, [allOpinions]);
  useEffect(() => {
  ...
  useEffect(() => {
    instance.get(`/negocios/${id}`)
      .then((anfitrion) => {
        ...
        setOpinion({
          ...opinion,
          id_negocio: anfitrion.data.id,
          rol_receptor: anfitrion.data.rol
        });
      })
      .catch((err) => console.log(err))
      ...
      <>
        ...
        >
          {(!envioOpinion) ? (
            ...
            <div className="modalCargaCargando">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-rotate-clockwise-2" width="44" height="44" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ffffff" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M9 4.55a8 8 0 0 1 6 14.9m0 -4.45v5h5" /><line x1="5.63" y1="7.16" x2="5.63" y2="7.17" /><line x1="4.06" y1="11" x2="4.06" y2="11.01" /><line x1="4.63" y1="15.1" x2="4.63" y2="15.11" /><line x1="7.16" y1="18.37" x2="7.16" y2="18.38" /><line x1="11" y1="19.94" x2="11" y2="19.95" /></svg>
                </div>
            </div>
          ) 
          }
        ...
        <section id="InfOpinionesAnfitrion">
          ...
          <section className="contOpiniones">
            <center>
              <div className="opCantidad" onClick={() => setShowModalCant(!showModalCant)}>
            ...
            <center>
              <div className="opCalidad" onClick={() => setShowModalCali(!showModalCali)}>
            ...
            <center>
              <div className="opPrecio" onClick={() => setShowModalPrec(!showModalPrec)}>
            ...
            <center>
              <div className="opSeguridad" onClick={() => setShowModalSeg(!showModalSeg)}>
            ...
            <center>
              <div className="opAmbiente" onClick={() => setShowModalAmb(!showModalAmb)}>
